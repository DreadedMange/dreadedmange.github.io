...
function createPerformanceChart(a){const b=document.getElementById("performanceChart").getContext("2d"),c=Object.keys(a),g=c.map(f=>a[f].correct),d=c.map(f=>a[f].incorrect);return new Chart(b,{type:"bar",data:{labels:c,datasets:[{label:"Correct",data:g,backgroundColor:"rgba(75, 192, 192, 0.5)",borderColor:"rgba(75, 192, 192, 1)",borderWidth:1},{label:"Incorrect",data:d,backgroundColor:"rgba(255, 99, 132, 0.5)",borderColor:"rgba(255, 99, 132, 1)",borderWidth:1}]},options:{scales:{y:{beginAtZero:!0}},
...
function updateGraphAndTable(a){var b=Object.keys(a).reduce((e,h)=>e+a[h].correct,0),c=Object.keys(a).reduce((e,h)=>e+a[h].incorrect,0),g=0<b||0<c?b/(b+c)*100:0,d=Object.keys(a).reduce((e,h)=>e+a[h].secondsCorrect,0),f=Object.keys(a).reduce((e,h)=>e+a[h].secondsIncorrect,0);b=0<b?d/b:0;c=0<c?f/c:0;document.getElementById("hand_category_table").innerHTML=`
<tr>
<th>Hand Category</th>
<th>Correct</th>
<th>Incorrect</th>
...
"1. Pick your cards"}
function renderAlternateCards(a){var b=document.getElementById("hole"),c=document.getElementById("board");b=b.getElementsByClassName("card");const g=c.getElementsByClassName("card");c=f=>a.some(e=>e.rank===f.rank&&e.suit===f.suit);for(var d of b)b={rank:d.dataset.rank,suit:d.dataset.suit},d.classList.contains("selected")?(d.classList.remove("selected"),c(b)?d.classList.add("selected-right"):d.classList.add("selected-wrong")):c(b)?d.classList.add("should-have-selected"):d.classList.add("unselected-right");for(const f of g)d=
{rank:f.dataset.rank,suit:f.dataset.suit},f.classList.contains("selected")?(f.classList.remove("selected"),c(d)?f.classList.add("selected-right"):f.classList.add("selected-wrong")):c(d)?f.classList.add("should-have-selected"):f.classList.add("unselected-right")}
function allSelectedCardsAreCorrect(a){var b=document.getElementById("hole"),c=document.getElementById("board");b=b.getElementsByClassName("card selected");c=c.getElementsByClassName("card selected");return 2!==b.length||3!==c.length?!1:[...b,...c].every(g=>{var d=g.dataset.rank,f=g.dataset.suit;return a.some(e=>e.rank===d&&e.suit===f)})}
document.addEventListener("DOMContentLoaded",()=>{const a=document.getElementById("hole"),b=document.getElementById("board"),c=document.getElementById("ConfirmBtn"),g=document.getElementById("DrawBtn");var d=document.getElementById("ResetStats");let f=!0;totalSeconds=0;const e=()=>{const k=a.getElementsByClassName("selected").length,l=b.getElementsByClassName("selected").length;c.disabled=5!==k+l};d.addEventListener("click",()=>{"texas"==gameType?localStorage.removeItem("texas_handCategoryTable"):
