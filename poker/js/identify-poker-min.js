...
function updateGraphAndTable(a){var b=Object.keys(a).reduce((e,k)=>e+a[k].correct,0),c=Object.keys(a).reduce((e,k)=>e+a[k].incorrect,0),f=0<b||0<c?b/(b+c)*100:0,d=Object.keys(a).reduce((e,k)=>e+a[k].secondsCorrect,0),g=Object.keys(a).reduce((e,k)=>e+a[k].secondsIncorrect,0);b=0<b?d/b:0;c=0<c?g/c:0;document.getElementById("hand_category_table").innerHTML=`
<tr>
<th>Hand Category</th>
<th>Correct</th>
<th>Incorrect</th>
...
${Object.keys(a).map(e=>{const k=a[e].correct,h=a[e].incorrect;var l=k+h;l=0<l?(k/l*100).toFixed(0):"-";const p=a[e].secondsCorrect%60,n=Math.floor(a[e].secondsIncorrect/60),m=a[e].secondsIncorrect%60,q=a[e].secondsCorrect/k,r=a[e].secondsIncorrect/h;return`
<tr>
  <td>${e}</td>
  <td>${k}</td>
  <td>${h}</td>
  <td>${l}%</td>
    <td>${Math.floor(a[e].secondsCorrect/60)}:${10>p?"0":""}${p}</td>
    <td>${isNaN(q)?"-":q.toFixed(1)}</td>
    <td>${n}:${10>m?"0":""}${m}</td>
...
document.addEventListener("DOMContentLoaded",()=>{const a=document.getElementById("hole"),b=document.getElementById("board"),c=document.getElementById("ConfirmBtn"),f=document.getElementById("DrawBtn");var d=document.getElementById("ResetStats");let g=!0;totalSeconds=0;const e=()=>{const h=a.getElementsByClassName("selected").length,l=b.getElementsByClassName("selected").length;c.disabled=5!==h+l};d.addEventListener("click",()=>{"texas"==gameType?localStorage.removeItem("texas_handCategoryTable"):
...
"full house":{correct:0,incorrect:0,secondsCorrect:0,secondsIncorrect:0},"four of a kind":{correct:0,incorrect:0,secondsCorrect:0,secondsIncorrect:0},"straight flush":{correct:0,incorrect:0,secondsCorrect:0,secondsIncorrect:0}};updateGraphAndTable(handCategoryTable)});d=h=>{if(g&&h.target.classList.contains("card")){var l="omaha"==gameType?"hole"==h.target.parentElement.id?2==a.getElementsByClassName("selected").length?!0:!1:3==b.getElementsByClassName("selected").length?!0:!1:5==document.getElementsByClassName("selected").length?
!0:!1;(h.target.classList.contains("selected")||0==l)&&h.target.classList.toggle("selected");e()}};a.addEventListener("click",d);b.addEventListener("click",d);c.addEventListener("click",()=>{g=!1;c.disabled=!0;f.disabled=!1;clearInterval(timerVar);const h=totalSeconds;var l=a.getElementsByClassName("selected"),p=b.getElementsByClassName("selected");if("texas"==gameType)var n=bestHoldemHand(Board,Hole);else if("omaha"==gameType)n=bestOmahaHand(Board,Hole);else throw Error("data-game-type attribute must be set to 'texas' or 'omaha'");
l=[...l,...p].map(r=>({rank:r.dataset.rank,suit:r.dataset.suit}));var m=scoreHand(l);p=scoreHand(n);let q;m.score===p.score?(createConfetti(),m=!0,n=l,q="Correct!:"):(m=!1,q="Wrong!:");updateTable(m,n,h);renderAlternateCards(n);renderHandScore(l,q,"score_user");m?document.getElementById("score_true").innerHTML="":renderHandScore(n,"Best Hand:","score_true");handCategory=categories[p.category];"texas"==gameType?(handCategory="holdem_"+handCategory+"_"+(m?"correct":"incorrect"),eventCategory="texas_hand"):
"omaha"==gameType&&(handCategory="omaha_"+handCategory+"_"+(m?"correct":"incorrect"),eventCategory="omaha_hand");gtag("event","identify_hand",{event_category:eventCategory,event_label:handCategory,value:h})});f.addEventListener("click",()=>{g=!0;f.disabled=!0;drawHand();e();clearInterval(timerVar);totalSeconds=0;document.getElementById("timer").innerHTML="00:00";timerVar=setInterval(countTimer,1E3)});d=document.getElementsByClassName("collapsible-button");for(var k=0;k<d.length;k++)d[k].addEventListener("click",
function(){var h=this.getAttribute("data-target");h=document.getElementById(h);this.classList.toggle("active");h.style.maxHeight?(h.style.maxHeight=null,h.style.overflow="hidden"):(h.style.maxHeight=h.scrollHeight+"px",h.style.overflow="auto")});document.getElementById("setting_use_timer").addEventListener("change",function(){localStorage.setItem("setting_use_timer",this.checked)});document.getElementById("setting_more_straights").addEventListener("change",function(){localStorage.setItem("setting_more_straights",
this.checked)});document.getElementById("setting_use_timer").checked="true"==localStorage.getItem("setting_use_timer");document.getElementById("setting_more_straights").checked="true"==localStorage.getItem("setting_more_straights");document.addEventListener("visibilitychange",function(){"hidden"===document.visibilityState?clearInterval(timerVar):timerVar=setInterval(countTimer,1E3)});drawHand();e();f.disabled=!0;if("texas"==gameType)d=localStorage.getItem("texas_handCategoryTable");else if("omaha"==
gameType){if(d=localStorage.getItem("omaha_handCategoryTable"),old_table=localStorage.getItem("handCategoryTable"))d?localStorage.removeItem("handCategoryTable"):(localStorage.setItem("omaha_handCategoryTable",d),localStorage.removeItem("handCategoryTable"),d=localStorage.getItem("omaha_handCategoryTable"))}else throw Error("data-game-type attribute must be set to 'texas' or 'omaha'");d&&(handCategoryTable=JSON.parse(d));updateGraphAndTable(handCategoryTable)});
